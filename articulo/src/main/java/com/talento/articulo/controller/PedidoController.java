package com.talento.articulo.controller;
...
import java.util.Map; // Para devolver mensajes de error
import java.util.Optional;

@RestController
@RequestMapping("/api/pedidos") // Definola URL base para los pedidos
public class PedidoController {
    ...
    // POST /api/pedidos
    // Corresponde a 'crearPedido()' de GestionSistema
    ...
    public ResponseEntity<?> crearPedido(@RequestBody List<LineaPedido> lineasDelRequest) {
        try {
            ...
            // Si la excepción EstockInsuficiente se lanza, devolve un 400 Bad Request
    ...
    // Para actualizar solo el estado de un pedido. Se espera un JSON como {"estado": "CONFIRMADO"}
    @PutMapping("/{id}/estado")
    public ResponseEntity<Pedido> actualizarEstadoPedido(@PathVariable Long id, @RequestBody Map<String, String> body) {
        String nuevoEstado = body.get("estado"); // Obtener el valor de la clave "estado"
